...
const Details = () => {
   ...
   useEffect(()=>{
    const filter = detailProd.filter(product=>parseInt(product.product_id)===parseInt(id))
    setDetails(filter)
    console.log
   },[detailProd])
        ...
        details.map(detail=>
            ...
            <div className="hero-content flex-col lg:flex-row">
              ...
              <div className='text-left'>
                ...
                <div className="badge badge-outline">{(detail.availability?"In Stock":"Out of stock")}</div>
                ...
                <Stack spacing={1}>
      <Rating name="half-rating-read" defaultValue={parseInt(detail.rating)} precision={0.5} />
     
    </Stack>
                <button onClick={()=>addToCart(detail)} className="btn btn-primary">Add To Cart</button>
              </div>
            </div>
          </div>
        )
